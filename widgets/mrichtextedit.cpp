...
void MRichTextEdit::textStyle(int index) {
    ...
    if (index == ParagraphMonospace) {
        ...
        fmt.setFontFamilies(QStringList() << "DejaVu Sans Mono");
...
QString MRichTextEdit::toHtml() const {
    ...
    s = s.replace(QRegularExpression("(<[^a][^>]+>(?:<span[^>]+>)?|\\s)([a-zA-Z\\d]+@[a-zA-Z\\d]+\\.[a-zA-Z]+)"), QStringLiteral("\\1<a href=\"mailto:\\2\">\\2</a>"));
    // convert links
    s = s.replace(QRegularExpression("(<[^a][^>]+>(?:<span[^>]+>)?|\\s)((?:https?|ftp|file)://[^\\s'\"<>]+)"), QStringLiteral("\\1<a href=\"\\2\">\\2</a>"));
    return s;
}
